...
- 准备实现 `/kdev-memory-export-md` 命令
- 想理解为什么 kdev-memory 不引入 JSONL 中间格式
- 设计 sanitize 规则
...
`/kdev-memory-export-md` 命令产出三个独立的 markdown 文件（输出位置：`.kdev/memory/dataset/`），每个 **self-contained**（便于外部消费 / 分享 / 上传）：
...
def export_markdown_slices(kdev_dir: Path, out_dir: Path):
    steps = parse_entries(kdev_dir / "执行日志.md", r"^##\s+Step\s+\S+")
...
| 用户说 | skill 动作 |
|---|---|
| "导出蒸馏数据 / export-md" | 走 `/kdev-memory-export-md`，产出 3 类切片包 |

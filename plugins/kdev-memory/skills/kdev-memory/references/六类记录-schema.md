...
> ⚠️ **Phase 2 · C1（Q 20260625-173847-ly1989abc）**：叙事 Step 的**物理主账已是 `执行日志.jsonl`**（[hooks/lib/step_log.py](../../hooks/lib/step_log.py) 读写，append-only + validate 7 hard-gate）；新 Step 走 **step-recorder 落 jsonl**（[agents/kdev-step-recorder.md](../../agents/kdev-step-recorder.md)），日总结由 [daily_render.py](../../hooks/lib/daily_render.py) 从 jsonl 确定性渲染（承重墙）。下面的 markdown schema 是 Step 的**逻辑/渲染形态**——**段语义完全不变**，但落盘物理主账是 JSONL。
>
> **历史 Step 留在 `执行日志.md`（冻结，不再写入），经 [step_dualread.py](../../hooks/lib/step_dualread.py) 与 jsonl 永久 dual-read（md∪jsonl 并集）**——存量 md **不迁移**、md-read **不退**（这是相对 ieidev jsonl-only 硬切的 deliberate 分叉）。
>
> 🚫 **不要再手写 `## Step` 进 `执行日志.md`**——新 Step 一律走 step-recorder 落 `执行日志.jsonl`。手写进冻结的 md 既绕过 7 hard-gate 校验、又会被当历史条目而非主账。

**何时写**：每完成一个有明确边界的工作单元（一个任务、一次评审、一次实验）就追加一条（落 `执行日志.jsonl`）。粒度比 commit 粗、比迭代细。
...
| 条目 | 文件 | 内容要点 |
|---|---|---|
| 1 | 执行日志.jsonl Step 评分段 | 顺畅度 4/5，用户评价段记项目部分（subject 隐含 = project） |

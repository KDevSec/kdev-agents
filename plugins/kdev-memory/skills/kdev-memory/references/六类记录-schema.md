...
- 第一次写某类记录、不确定字段格式时
- 需要查"模型自评 / 用户评分 / 双评分锁定 / 当前状态 frontmatter 字段"等具体要求时
- 审阅历史记录条目是否规范时
...
为支持多 worktree 并发场景，Step ID 从 v0.11 起加分支前缀。详见 [SKILL.md](../SKILL.md) 的「多 worktree 并发场景：Step ID 加分支前缀（v0.11+）」一节。
...
**闸门触发时机**：看到任一停顿信号时，skill 主动走 Step 四段流程（执行事实 → 模型自评 → 公布 → 追问用户评分 → 评分差异分析）。这就是"何时把 todo list 里的一组 completed 折叠成一条 Step 条目"的判断依据。

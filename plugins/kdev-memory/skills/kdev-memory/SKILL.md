---
name: kdev-memory
description: 在多迭代、多决策的长周期工程项目中，为智能体建立持久的"工程记忆"制度——每步完成、每次踩坑、每个决策、每次评分实时落盘到 `.kdev/` 规范文件，用作跨会话续航 + 未来 skill/方法论提炼的原料库。核心机制：**持续实时记录 + 快速汇总输出**——用户要每日总结时，智能体从 `.kdev/` 当天条目聚合拼装，绝不翻会话记录。触发时机：用户说"建立工程记忆 / 记忆机制 / 给项目加 .kdev"开始初始化；或说"写今天的总结 / 生成每日汇总 / 总结一下今天 / 交接给明天 / 今日工作总结"时快速整合当天记录并输出；或会话即将被压缩（PreCompact 事件）时确保关键 Step/决策/踩坑落盘；或新会话启动时 SessionStart hook 注入 `<kdev-memory-brief>` 提示有 `.kdev/` 记忆 + 待处理 WARN-*.md 或 checkpoint 文件；或用户问"昨天做到哪了 / 之前我们聊到什么 / 上次进度 / 继续上次的工作 / 恢复上下文"需要从 `.kdev/` 回读；或新开多迭代项目需要跨会话续航；或用户抱怨"每次新会话都要重新解释上下文 / 上次的坑又踩了"。不管状态机或流程编排，只管"该记什么、何时记、怎么记、怎么保住"。
---
...
智能体在项目推进过程中，**每做完一步、踩一次坑、做一次决策、接到一次用户评分，就立刻把当条记录写进 `.kdev/` 对应文件**。这是持续进行的后台动作——不是攒到会话最后一次写完，更不是等用户提醒才记。
...
读 .kdev/执行日志.md  ──┐
读 .kdev/决策日志.md  ──┤
读 .kdev/踩坑日志.md  ──┼── 按今天日期筛选条目 → 拼装 → 写入 每日汇总/YYYY-MM-DD.md
读 .kdev/改进建议.md  ──┤
读 .kdev/当前状态.md  ──┘
```
...
项目根目录下建立记忆根（默认 `.kdev/`，可由用户重命名为 `.memory/` `.v-agent/` 等——与本 skill 的名字无关）：
...
**执行顺序**：建空骨架文件 → 向用户抛 Q-001 → 用户答 → 写入 `.kdev/决策日志.md` 的 Q-001 条目 → 把用户选择同步到 CLAUDE.md 触发规则段的"编号规则"那一行。骨架先建好，用户才有"这些文件是什么"的实感，对 Q-001 的选择才不是抽象的。
...
- 修改 frontmatter**不需要征求用户许可**——和其他 `.kdev/` 文件一样，授权一次后自维护
...
走项目流程时，必须**实时**维护 `.kdev/` 下的记录文件。**不需要征求用户许可即可写入这些文件。**

🔴 **实时铁规**：每做完一步、踩一次坑、做一次决策、接到一次评分，**立刻**追加一条到对应文件。不要攒到会话结束或每日汇总时才回忆着写——那样记录就失真了。

🔴 **每日汇总不翻会话**：用户要求"生成每日总结"时，直接从 `.kdev/` 读当天条目聚合，不要回翻会话上下文、不要让用户复述。如果 `.kdev/` 空了，坦白报告"今天实时落盘没跟上"，不要编造。

🔴 **新会话进入项目先看 `.kdev/WARN-未记录-*.md`**：这是 SessionEnd hook 在上次会话"有变更但没落盘"时留下的兜底警告文件。看到就优先处理：读文件里的变更快照、向用户确认那些改动对应什么 Step、把补记写进执行日志，然后删除 WARN 文件。不要忽略它往下做别的事。
...
🔴 **看到 `.kdev/checkpoints/压缩前-*.md`**：这是 PreCompact hook 在会话压缩前留下的快照。一般情况不需要主动看；只在以下场景读：(a) 用户说"上次压缩前的细节"；(b) 当前 `.kdev/` 文件有缺失但 checkpoint 里有原文；(c) 执行日志有空当期且同时段有 checkpoint（说明压缩时没来得及落盘）。Checkpoint 7 天后 PreCompact hook 自动清理，不用手工维护。

🔴 **每次完成 Step 都要顺手更新 `.kdev/当前状态.md` 的 frontmatter**：至少改 `current_step` + `last_updated`。有新开的 Q-NNN 或 G-NNN 也要加到对应字段。不是攒到每日汇总时才改——frontmatter 字段就是给 SessionStart hook 和下一次会话看的，延迟更新等于下次启动就拿到过期数据。

### 触发规则

| 触发时机 | 记录目标 | 格式要求 |
|---------|---------|---------|
| 每次需要人做决策 | `.kdev/决策日志.md` | 追加 Q-NNN：问题、选项、用户选择、理由 |
| 每次踩坑/绕路/报错 | `.kdev/踩坑日志.md` | 追加 G-NNN：现象、原因、解决 |
| 每步完成后 | `.kdev/执行日志.md` | 追加 Step N：执行事实 + 模型自评（含扣分项）+ 用户评分 |
| 每步结束后 | 主动向用户征询评分 | 模型先自评+列事实，再让用户打分；**用户评分当场采集，不允许延后** |
| 会话结束前 | `.kdev/每日汇总/YYYY-MM-DD.md` | 汇总当天工作 |
| 评分差值 ≥ 2 或反复负面反馈 | `.kdev/改进建议.md` | 追加 R-NNN：原话 + 事实 + 评分 + 初步诊断（不强制在项目内执行） |
| （可选）项目自愿立硬规 | `.kdev/方法论铁规.md` | 用户明确要求才写，默认不用 |
| 流程状态变更 | `.kdev/当前状态.md`（含 frontmatter） | 更新 body + **同步 frontmatter 字段**（phase/iteration 等） |
| 每完成 Step 后顺手 | `.kdev/当前状态.md` frontmatter | 更新 `current_step` + `last_updated`；如有新 Q/G 同步列表 |
| 发现 `.kdev/WARN-未记录-*.md` | 优先处理，不跳过 | 读变更快照 → 向用户核对 → 补记 Step → `rm` 该 WARN 文件 |
| 发现 SessionStart 注入的 `<kdev-memory-brief>` | 先处理摘要里的⚠️条目 | 按提示读 WARN/checkpoint/待处理项 → 完成后再做用户任务 |
| 发现 `.kdev/checkpoints/压缩前-*.md` 且当前需要回溯 | 按需 Read | 只在用户询问"压缩前细节"或本地 .kdev/ 缺失时读 |
...
- 续航：新会话时提到"看下昨天做到哪了 / 上次我们讨论到什么"—— skill 会被召唤来读 `.kdev/`
...
- **Stop hook（软提醒，默认）**：Claude 每次要停下时检查 `.kdev/` 状态，向下一轮上下文注入文本提醒（今天无汇总 / 汇总过时 / 执行日志今天空）。
- **Strict 模式（条件阻塞，opt-in）**：项目根 `touch .kdev/strict` 即启用。执行日志今天空 + 工作区有变更 + (≥2 文件变更 或 命中里程碑白名单如 `specs/**/*.md`) → `exit 2` 阻塞 Stop，Claude 必须落盘后才能真正结束。带 `stop_hook_active` 保护不会无限循环。`rm .kdev/strict` 关闭。
- **PostToolUse hook**：Claude 用 `Write/Edit/MultiEdit` 命中里程碑白名单（Spec Kit 产物、契约 YAML、方法论铁规）时，立刻提醒追加 Step 到执行日志。日常源码编辑不打扰。
- **SessionEnd hook（兜底）**：会话真正结束（客户端关闭、切项目）时，若今天执行日志无条目但工作区有变更 → 写 `.kdev/WARN-未记录-YYYY-MM-DD.md` 文件，列出变更快照。下次进入项目 CLAUDE.md 触发规则段会让 Claude 看到并提醒补记。
...
Step 3：写入 .kdev/每日汇总/YYYY-MM-DD.md
         - 文件已存在则追加（一天多次会话累积）
         - 文件不存在则新建

Step 4：更新 .kdev/当前状态.md
```

**必须遵守**：
- 整个过程应当**快速**（几秒到十几秒），因为只是文件 I/O + 拼装，不是推理
- **不要回头翻会话记录**：会话上下文只是"刚写进文件的内容的副本"，读文件才是准确的
- **不要让用户复述**："你今天做了什么？"——不，用户已经在过程中给过实时评分和反馈了，都在 `.kdev/` 里
- 如果 `.kdev/` 里今天条目为空（说明实时落盘没跟上），**坦率报告**"今天 `.kdev/` 里没有记录，无法聚合——这是 skill 失效的信号，建议下次启用 hook 或加强触发规则段"，**不要**凭印象补写

### 为什么这个设计重要？

- **决定 skill 是否成立**：如果每日汇总靠回忆写、每步记录靠总结阶段回填，这个 skill 就退化成"会话结束前写点总结"——毫无价值。区别不在于"有没有文件"，而在于"文件里的内容是否是实时的、准确的、不偏向的"。
- **让下游 skill 作者能用**：未来的 skill 作者读 `.kdev/` 时看到的必须是**当时当刻的原始信号**，不是事后美化的总结。美化后的总结失去了"智能体和用户当下的张力"，也就失去了提炼新 skill 的原料价值。

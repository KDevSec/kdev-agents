...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/hooks/lib/init-gitignore.py"
```
...
**单人项目可选托管**：如果确实希望 `.kdev/` 也进 git，设置环境变量 `KDEV_GIT_TRACK=1` 再执行 init-gitignore.py，或手动 `rm` 掉 .gitignore 里的 `.kdev/` 行。但**强烈不推荐**用于多会话 / 多成员场景（会触发 merge 冲突、Step 编号竞态、基线分叉，详见开发历程 v0.7 章节）。

...
> **Phase 2 · C1（Q 20260625-173847）**：叙事 Step 物理主账 = `执行日志.jsonl`（append-only，step-recorder 落盘）；历史 Step 留在 `执行日志.md`（**冻结**，不再写入），经 [hooks/lib/step_dualread.py](../../hooks/lib/step_dualread.py) 与 jsonl **永久 dual-read（md∪jsonl 并集）**。**存量 md 不迁移、md-read 不退**——这是相对 ieidev 硬切（jsonl-only、丢 md）的 **deliberate 分叉**。决策/踩坑/改进/skill-feedback/每日汇总/当前状态仍永久 markdown 主存，本次迁移完全不碰。
...
长项目主文件膨胀时，采用**人工触发 + Claude 提醒**的切档制度。核心规则：踩坑/决策按季切、改进建议不切；搬家不删除，编号保留，切档前征得用户同意。Stop hook 检测跨期时提醒。**执行日志按月切档已下线**（Phase 2 · C1，Q 20260625-173847：叙事 Step 走 `执行日志.jsonl` append-only 主账，无 jsonl 月度 rotation；历史 `执行日志.md` 冻结·经 step_dualread 永久 dual-read，不再切档），故 `archive_hint.py` 的 Step 月度归档 gate 已 off，决策/踩坑季度切档仍照常。
...
```
Step 1（必跑·承重墙）：跑确定性渲染器，从 执行日志.jsonl 主账落出当日汇总骨架
         python3 hooks/lib/daily_render.py --date YYYY-MM-DD [--root .kdev/memory]
         - 默认写入 .kdev/memory/每日汇总/YYYY-MM-DD.md（已存在则追加，一天多次会话累积；可重入）
         - 加 --stdout 则只打印不落盘（先看再决定）
         - 自动出全 5 段：完成的工作（含「模型 N / 用户 M」双评分摘要）/ 未完成项 /
           明日计划占位 / 本日新增 Q/G/R 索引（双认时间戳形 + legacy ID）/ 负面评价观察（delta≤-1）
         - flat / scoped 布局都对（走 scope.shared_dir）；Q/G/R 仍读对应 md
         ⚠️ 不要再靠 LLM 手动 Read 多个 md 文件回忆拼装——那是被这一步取代的旧路径

Step 2（可选·LLM 叠加层）：在 daily_render 落出的骨架上润色
         - 仅在用户要叙事概述 / 填「明日计划」占位 / 补人读说明时才做
         - 是叠加层，永不进关键路径；不做也是一份可独立成稿的合格汇总
         - 润色时直接编辑 daily_render 写出的同一 .kdev/memory/每日汇总/YYYY-MM-DD.md

Step 3：更新 .kdev/memory/当前状态.md
```

**必须遵守**：
- Step 1 的确定性渲染应当**秒级**完成（脚本只做 JSONL/md I/O + 拼装，不是推理）；可选的 LLM 润色才算"加料"

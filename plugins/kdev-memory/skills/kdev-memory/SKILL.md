...
**3. 自动触发（Hook，可选增强）**
Claude Code 支持 `settings.json` 里配 hook，在特定事件发生时强制执行命令或提醒 Claude。对本 skill 最有用的两种：

- **Stop hook**：Claude 每次要停下返回时触发。可以注入一条提醒文本"检查本步是否该追加执行日志 / 是否该写每日汇总"，Claude 看到后会主动处理。
- **SessionEnd hook**：会话结束时触发。可以跑一段 bash 检查 `.kdev/每日汇总/$(date +%F).md` 是否存在，不存在就打印提醒。

**最小 hook 样板**（加到 `~/.claude/settings.json` 或项目 `.claude/settings.json`）：
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "if [ -d .kdev ] && [ ! -f .kdev/每日汇总/$(date +%F).md ]; then echo '提醒：今天还没写每日汇总，如果本次是当日最后一次会话，调用 kdev-memory skill 生成汇总。'; fi"
          }
        ]
      }
    ]
...
这个 hook 的作用：每次 Claude 要结束一轮回答时，如果项目启用了 `.kdev/` 且今天的汇总还没写，就打印一条提醒。Claude 读到提醒会自行调用 skill。

**边界说明**：hook 只能运行 shell 命令、不能让 Claude 做智能判断——它的作用是**戳一下 Claude 的注意**，真正的决策（该不该写、写什么）仍由 skill 负责。不要想用 hook 代替 skill。

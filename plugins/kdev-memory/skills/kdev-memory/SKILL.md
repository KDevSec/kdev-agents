---
name: kdev-memory
description: 在多迭代、多决策的长周期工程项目中，为智能体建立持久的"工程记忆 + 智能召回"制度——每步完成、每次踩坑、每个决策、每次评分实时落盘到 `.kdev/memory/` 规范文件；用户再次提到相关话题时，UserPromptSubmit hook 自动召回 <kdev-memory-recall> 指针让 Claude 按需 Read 细节。用作跨会话续航 + 避免重复踩坑 + 未来 skill/方法论提炼的原料库。核心机制：**持续实时记录 + 快速汇总输出 + 命中即召回**——用户要每日总结时从 `.kdev/memory/` 当天条目聚合，绝不翻会话记录；用户提到 G/Step/铁规 里标过 triggers 的关键词时自动注入指针。触发时机：用户说"建立工程记忆 / 记忆机制 / 给项目加 .kdev"开始初始化；或说"写今天的总结 / 生成每日汇总 / 总结一下今天 / 交接给明天 / 今日工作总结"时快速整合当天记录并输出；或会话即将被压缩（PreCompact 事件）时确保关键 Step/决策/踩坑落盘；或新会话启动时 SessionStart hook 注入 `<kdev-memory-brief>` 提示有 `.kdev/memory/` 记忆 + 待处理 WARN-*.md 或 checkpoint 文件；或用户问"昨天做到哪了 / 之前我们聊到什么 / 上次进度 / 继续上次的工作 / 恢复上下文"需要从 `.kdev/memory/` 回读；或看到 `<kdev-memory-recall>` 注入提示"刚才踩过的坑 / 之前做过的决策 / 项目铁规"时按需 Read；或用户抱怨"每次新会话都要重新解释上下文 / 上次的坑又踩了"。不管状态机或流程编排，只管"该记什么、何时记、怎么记、怎么保住、怎么召回"。
---

...
在 `knowledge-graph.json` 中按节点 ID 约定（`file:<path>` / `document:<path>` 等，完整定义见 [conventions.md](../../references/conventions.md)），找所有 `documents` 类型且 source/target 之一是当前 doc 的边。

---
name: kdev-graph-build
description: 在当前项目构建代码知识图谱（先调 UA /understand 建基础图，再用 kdev-ingestor 灌入 kdev-secure-coding 的安全规范节点）。触发时机：用户说"建图谱 / 建立代码图谱 / 跑 understand / 灌安全规范 / 准备追溯"，或第一次使用 trace-security / security-impact / doc-code-sync 但 .understand-anything/knowledge-graph.json 不存在时。
---
...
并提示：
- 想看「这条规范在哪些代码里实现」→ `/trace-security`
- 想看「改这段代码会影响哪些规范」→ `/security-impact`
- 想看「文档代码哪些脱节」→ `/doc-code-sync`

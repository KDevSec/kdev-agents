...
把"对话里点头 + IDE 里再点一次"两道确认收成一道。你在对话里**显式授予「自主推进」**后，AI 给每条 push 盖一个 `# 自主推进` 令牌注释，hook 认令牌就**跳过 IDE 弹窗**：

```bash
git push # 自主推进          # ← 带令牌，hook 放行，不弹窗
git push                     # ← 无令牌（手动模式），照常弹窗兜底
git push --force # 自主推进   # ← 裸 --force 即便带令牌也仍弹窗（不可逆，留最后一道关）
```

- **触发词（你说）**：自主推进 / 自动推进 / 自主执行 / 自动执行 / 无人值守 / 全自动 / 自动跑 / 自己跑完 / 一路到底 / 不用问我 / 不用每次确认 —— 说任一个即进入自主推进态
- **撤销词（你说）**：停 / 暂停 / 手动 / 我来确认 / 退出自主 / 接管 —— 立刻回到每次弹窗
- **比 `off` 更安全**：`off` 是全局静默（AI 漏问擅自推也没拦截）；令牌只放行**带章**的 push，没盖章的 push（手动模式 / 误推）仍弹窗兜底
- **`--force` 永不放行**：裸 `--force` 即便在自主推进态也弹窗（要 `warn-force`/`ask` 档下才有此兜底；`off` 档本就全关）

> 令牌只是把你对话里给的授权机械地带到命令上。AI 受 SKILL.md 约束：没有你的触发词，不盖令牌；歧义则保守回落到弹窗。
...
- **不擅自 push** —— push 必须经用户对话授权；默认还要在 IDE 权限框里再确认，自主推进态（v0.4.0）可跳过 IDE 那道（裸 `--force` 除外）

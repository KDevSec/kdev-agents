...
| 项 | 值 |
|---|---|
| 文档性质 | **brainstorming 设计稿 v0.1**（待用户复核；本任务**只设计不实施**，不进 writing-plans）|
| lifecycle | design |
| 日期 | 2026-06-12 |
| 范围 | 把第 4 个员工**评审专家**从 sketch（搬 X3 的 checklist + 冲突仲裁）补到**实施级接入设计**：① 形态=canonical callee 员工 ② 本期实现「有 caller gate」的 6 评审能力子集 ③ 兑现 dev-engineer 3 deferred + req-architect 3 self gate 翻 reviewer-expert ④ 承载 MQ-2 第三方他评（同员工双 mode）⑤ 发函边界硬规。**defer**：核心 10 余 4（需求方向/迭代拆分/测试设计/测试执行）随 caller 员工建成补 · 扩展 6 默认关 · mode-2 记忆 Step 他评（blocked-on P-C1b）|
| 承 | [Q-011 阶段2 接入打法](../../../.kdev/memory/决策日志.md) · [Q-015 P-C1b/他评 reframe](../../../.kdev/memory/决策日志.md)（mode-2 形态 = workflow-batch）· **[Q-016 本设计拍板](../../../.kdev/memory/决策日志.md)** |
...
| 2 | **承载他评** | **同员工双 mode**：mode-1 项目产物评审（本期落地）+ mode-2 记忆 Step 他评（接口预留，blocked-on P-C1b）；评审专家 = §1.5.7/§1.5.1「他评智能体」实体 | roadmap「与评审专家可合一」；共享「独立读 ground-truth 打分」内核；mode-2 形态承 Q-015 = workflow-batch（§2.4 / §7）|
...
### 2.4 mode-2 形态 = workflow-batch（与 Q-015 / roadmap 对齐）⭐

[Q-015](../../../.kdev/memory/决策日志.md) 把「记忆 Step 他评 / MQ-2」reframe 成 **workflow-batch fan-out** 形态（commit 锚点切段 → 后台 fan-out N 个独立读者 → 批量出他评），roadmap §1.5.8 line 189/192 据此记「评审专家 形态 = workflow-batch fan-out」。

**本设计与之不矛盾、是细化**：评审专家**两 mode 两形态**——

- **mode-1 项目产物评审**（本期）= **callee 同步形态**：caller gate 发函、fan-out in-scope 评审能力、caller await handoff（§3）。
- **mode-2 记忆 Step 他评**（留接口）= **workflow-batch 异步形态**：on-demand 触发、commit 锚点切段 fan-out（Q-015 的 reframe），blocked-on P-C1b transcript 溯源。

roadmap line 189 的「workflow-batch」专指 mode-2 的他评承载；mode-1 的 gate 接入是 callee per-gate dispatch。同一员工、两触发形态。**本 spec 落地后回写 roadmap line 189 的「spec TBD」→ 指向本文件，并把「形态 = workflow-batch」澄清为「mode-2 = workflow-batch / mode-1 = callee per-gate」。**
...
## 7. 承载 MQ-2 第三方他评：同员工双 mode

| | mode-1 项目产物评审（本期落地）| mode-2 记忆 Step 他评（接口预留，blocked-on P-C1b）|
|---|---|---|
| 触发 | caller R3 review gate（§5）| kdev-memory on-demand（非 gate），workflow-batch（Q-015）|
| 形态 | callee 同步 fan-out | workflow-batch 异步 fan-out（commit 锚点切段）|
| 输入 | 产物 + standards | 真实 transcript + diff + step_id |
| 输出 | 评分表 + verdict → handoffs/reviewer/ | `### 第三方他评` block 写回执行日志该 Step，逮 `### 模型自评` confabulation |
| 复用内核 | 6 评审能力 | `reviewer-code`（读真实 diff/transcript 打分）|
| 记忆落点（§5.2）| 评审经验（评得对/松/紧）→ `/staff/reviewer/` `subject:review:<cap>` **F-NNN**，蒸馏 `dataset-review-by-capability` | `### 第三方他评` 入执行日志 Step |

**mode-2 本期只定义接口、不实现**——读 ground-truth 依赖 transcript 溯源，P-C1b 仍 defer（且需先过 §5.6 spike-gate）。spec 标 `blocked-on: P-C1b`。这样评审专家既兑现 §1.5.7/§1.5.1「他评智能体」（roadmap「与评审专家可合一」），又不被未就绪依赖拖住本期 mode-1 交付。Q-015 已论证：P-C1b 溯源「部分买下」MQ-2（杀事实性 confab），mode-2 他评补 self_eval 同进程偏差最后一公里。
...
**defer（声明在案，不本期）**：核心 10 余 4 评审能力（随 caller 员工建成）· 扩展 6 · mode-2 记忆 Step 他评（blocked-on P-C1b）· 跨员工直接发函硬规完整体（v1.5 硬规 2/7）· CQO 元监督。
...
**明确非目标（防镀金）**：本期不建测试评审/需求方向/迭代拆分能力（无 caller）· 不建扩展 6 · 不实现 mode-2 · 不引入 kdev-core standards 加载魔法（agent 直接 Read）· 不另设 X3 审查组长（仲裁 inline）· 不动 CQO/HUD。

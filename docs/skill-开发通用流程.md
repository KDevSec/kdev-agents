...
> 2. 一个真实项目 **kdev-memory** 从自用脚手架到插件 0.3.0 的 10 天实战故事，可作为每一步的"实例锚点"（详见 [kdev-memory 开发历程技术分享](kdev-memory-开发历程技术分享.md)）
...
> 📎 **kdev-memory 案例**：从 token-statistics 的 `.kdev/` 脚手架跑了 10 天 / 4 迭代后才抽成插件，完整时间线见 [kdev-memory 开发历程 §1](kdev-memory-开发历程技术分享.md#1-真实起源从-token-statistics-sprint-0-实战验证项目长出来的)。
...
> 📎 **kdev-memory 案例**：项目内建 `改进建议.md`，每次 skill 表现不如预期就追加一条（原始 prompt + 实际 vs 期望 + 用户原话）。积累若干条后迁移到 `evals/` 测试集——**现场证据直接变回归测试**。详见 [kdev-memory 开发历程 §6.1](kdev-memory-开发历程技术分享.md#61-双评分机制防止智能体的讨好式满分)。
...
> 📎 **kdev-memory 案例**：0.1 → 0.3 每一版都有具体催生点（如 0.2.0 来自"Spec Kit 跑两小时 Stop hook 没被读到"），详见 [kdev-memory 开发历程 §4](kdev-memory-开发历程技术分享.md#4-版本迭代故事真实的成长线)。
...
> 📎 **kdev-memory 案例**：memory 生态 6 家调研是 2026-04-08 做 KDev-Agent 融合架构时的副产品（早于 kdev-memory 这个概念 8 天）；0.2.0 的 PreCompact 专项调研才是"为版本而做"的二次调研。详见 [kdev-memory 开发历程 §5](kdev-memory-开发历程技术分享.md#5-横向调研我们从六家框架学到什么拒绝什么)。
...
> 📎 **kdev-memory 案例**：0.3.0 的 UserPromptSubmit 智能召回机制本质上是**给每条踩坑 / Step 条目加 description**（`triggers:` 字段）——让该触发的时候一定能触发。详见 [kdev-memory 开发历程 §6.3](kdev-memory-开发历程技术分享.md#63-triggers-字面匹配--sanitizedeterministic-且防误触)。
...
> 📎 **kdev-memory 案例**：0.3.0 实际用了七层 hook 覆盖不同会话阶段（SessionStart / UserPromptSubmit / Stop / Strict / PostToolUse / PreCompact / SessionEnd），完整对照见 [kdev-memory 开发历程 §4.3-4.4](kdev-memory-开发历程技术分享.md#43-020--六层防线--yaml-frontmatter) 和 [kdev-memory README 的 Hook 章节](../plugins/kdev-memory/README.md#hook-行为七层防线)。
...
- [kdev-memory 开发历程技术分享](kdev-memory-开发历程技术分享.md) —— 本文引用的真实案例故事（711 行）

...
适用于 `req-architect` 员工的 design-flow。3 评审 gate 复刻 kdev-design-flow 的 3 闸门，**已翻 `reviewer=reviewer-expert`**（兑现评审专家，spec v0.2 / Q-016）——编排到 gate **发函 `kdev-team:reviewer-orchestrator`**（in-scope cap：g-sr-review→sr / g-ar-proto-review→story+prototype / g-design-review→design），走上面「Reviewer-Expert Gate（已兑现）」的发函 6 步，verdict 由评审专家百分制评分表 + 双重通过条件聚合得出，caller `record-gate --by reviewer-expert`。

**L1 回退 self 自评**（env 受限/省 token）：L1 flow-config `reviewer: self` 时不发函，主控按 design-flow `skills/kdev-design-flow/references/review-gate-prompt.md` 的成功标准自评，`config.review_mode` 控档：

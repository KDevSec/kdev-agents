...
| **② 网页实时仪表盘** | 一屏全景（自包含 hud.html）| 想细看时打开：完成度 / 员工在忙啥 / 评审分数 / 监督告警 / 事件流 | **kdev-hud** |
...
| **评审流水 / 分数** | `events.jsonl` 的 gate 事件 | 各闸 PASS/FAIL + score |
| **CQO 告警** | `events.jsonl` 的 cqo/blocked 事件 | 合规率 + 待处理告警 |
...
- **评审流水管线**：各评审节点（绿=已过带分 / 黄=评审中 / 灰=待评）
- **CQO 监督条**：合规率 + 告警

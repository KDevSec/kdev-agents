...
# Part 2：续篇 — 从 0.3.0 到 0.5.1 的 7 轮 iteration（2026-04-22 ~ 04-23 两日）

> 前文收笔于 0.3.0（2026-04-20）。接下来两天里，kdev-memory 从 0.3.0 →
> 0.4.0 → 0.5.0 → 0.5.1，经历了 7 轮 skill-quality iteration。这段不是
> 新功能堆砌，而是**方法论层面的跨越**——从"写 skill 文档"到"用 eval 数据
> 驱动 skill 文档"。下面按真实时序记录。

## 14. 起点：0.3.0 之后，token-statistics 暴露的老问题

0.3.0 发出去之后的几天里，token-statistics 项目（kdev-memory 的"生养地"）出
了一个现实的状况：**skill 一直在升，但项目自己的 `CLAUDE.md` 还是 0.2.0 时代
写入时冻结的那份 snapshot**。迭代 5/6/7/8 里用户反复发现"模型写完自评就把 Step
认定完成了"的失守现象——追下去根因不是 skill 本体逻辑有问题，而是 CLAUDE.md
规则段已经跟 skill 实现细节脱节了。

同时，SKILL.md 自己也膨胀到了 **734 行**。每次 skill 被触发 Claude 都要扛着
这 700+ 行进入上下文——大多数场景根本用不上其中的 80% 细节。渐进式披露喊了
几个版本，但实际上没真正落地。

这两件事凑到一起，就有了两份 "2026-04-22" 的审计文档（见 `dev-notes/`）：
**CLAUDE.md 模板漂移审计** 和 **skill 使用记录与体验评分维度缺口**。前者是
本轮演进的主驱动。

## 15. 0.4.0（2026-04-21）：按期归档切档 + 单元测试层

0.3.0 的 triggers 智能召回让 `.kdev/memory/` 主文件随项目推进线性膨胀——半
年下来执行日志 1000+ 行、踩坑日志几十条 G-NNN，Claude Read 效率和 trigger
扫描性能都吃力。0.4.0 加了两个能力：

- **按期归档**：执行日志按月、踩坑/决策按季度归档到 `.kdev/memory/归档/`
  子目录。不设行数阈值（用日期边界判断更自然），Stop hook 跨期时输出 📦
  提醒、让用户确认再切档（不自动搬）
- **单元测试层**：`tests/test_trigger_match.py` 44 个 stdlib unittest，覆盖
  sanitize / triggers 解析 / 匹配 / TTL / glob 扫描 7 类核心逻辑

0.4.0 是标准的"补完现有架构的下一层"——没有结构性惊喜，但把基础设施补齐了。

## 16. 本 session 开篇：skill-quality eval 基础设施（iter-1 ~ iter-2）

2026-04-22 这天的工作不是从"加功能"开始的，而是**建立一条"skill-creator 式
的 eval 线"**——专门测 SKILL.md + references/ 的**行为质量**，独立于原有
`evals/evals.json`（测 hook 脚本的 trigger-match 召回）那条线。

### 16.1 为什么先做 eval 线

审计文档提到 5 类 14 条问题、§5.1 里 9 个改进建议按 P0/P1/P2 排优先级——但
正如前文 §7 "从'我觉得应该'到'数据驱动'" 的教训：**没有 eval 就没法判断哪条
建议真的需要做**。审计里 P0-1 "Step 完成闸门" 听起来很硬、很对——但是不是
真的改变 skill 行为？Claude 在 skill 里自己已经能推出这个规则，还是真的需要
显式写一章？

为了回答这类问题，必须有一条"场景 × skill 版本"的对照测试系统——这是
skill-creator 方法论的典型应用。

### 16.2 eval-quality 线的目录结构

```
evals/
├── README.md                 # 两条线分工说明
├── evals.json                # 原有 10 条 trigger-match 召回（0.2 vs 0.3 对比）
├── run-hook-selftest.sh      # 原有
├── fixtures/
│   ├── project-state/        # trigger-match 线共享 fixture
│   └── skill-quality/        # skill-quality 线每场景独立 fixture
│       ├── eval-0-init/
│       ├── eval-1-daily-summary/
│       ├── eval-2-rule-upgrade/
│       ├── eval-3-archive/
│       ├── eval-4-merge-conflict/
│       ├── eval-5-missing-data/
│       ├── eval-6-cross-session-resume/
│       ├── eval-7-warn-file/
│       ├── eval-8-claude-md-drift/
│       ├── eval-9-step-half-complete/
│       └── eval-10-step-gate/     (discriminating，临时 fixture)
└── skill-quality/
    ├── evals.json            # 场景 prompt + fixture 路径 + assertions（单一真相源）
    └── iterations/           # 每次跑一轮 eval 就新建一个子目录
        ├── 20260422-01-baseline-3-scenarios/
        ├── 20260422-02-expanded-6-scenarios/
        ├── 20260422-03-decoupled-claude-md/
        ├── 20260422-04-claude-md-lint/
        ├── 20260422-05-eval6-coverage/
        ├── 20260423-06-step-completeness-lint/
        └── 20260423-07-p0-1-discriminating/
```

两条 eval 线的分工原则：
- `evals/evals.json` **测 hook 脚本**——改 hook 代码才跑
- `skill-quality/evals.json` **测 SKILL.md + references 的行为质量**——改 skill 文档才跑

### 16.3 Phase 1 重构：SKILL.md 734 行 → 254 行（iter-1、iter-2）

拆分动作按 "progressive disclosure" 原则：

- SKILL.md 保留核心原则 + 各场景指针（≤300 行）
- 6 个 references 承接细节：
  - `六类记录-schema.md` — Q/G/Step/汇总/R/铁规/当前状态 详细格式
  - `triggers-写法.md` — 召回锚写法规范
  - `初始化-claude-md-模板.md` — CLAUDE.md 触发规则段全文
  - `切档与归档.md` — 按月/季度搬家流程
  - `规则升级流程.md` — R-NNN → 宪章流程
  - `自动化机制-hooks.md` — 各 hook 行为细节

SKILL.md description 从 ~1400 字精简到 ~640 字（保留 5 个最高频触发场景）。

iter-1 跑了首轮 3 场景探索，iter-2 扩展到 6 场景 + 升级 assertions 做完整
验证：**12 runs pass rate 100% vs 100%**（行为零损失），**tokens 平均省
-19.6%**。每个 with_skill run 精准读取最相关的 reference 文件（eval-3 切档
读 `切档与归档.md`、eval-4 合并冲突读 `初始化-claude-md-模板.md`），证明
渐进式披露真正在按场景动态工作、不是"文件拆小了"的表面功夫。

iter-2 同时发现：eval-4（合并冲突）/ eval-5（数据缺失）新老 skill 行为**完全
一致**——两个版本都正确 pause / 拒绝编造。这组数据为后续 iter-7 埋下了
"skill 本体已够硬"的伏笔。

## 17. iter-3：CLAUDE.md 接口 / 实现解耦

### 17.1 问题诊断

审计 §2 对照表指出：老 CLAUDE.md 规则段承担了错位职责——既讲"何时召唤 skill"
的**路标**信息，又塞了 17 行触发规则表、评分机制要点、编号规则、hook 行为
响应表等**实现细节**。每次 skill 升级，实现细节会变，规则段就漂移。

### 17.2 解耦方案

把 CLAUDE.md 规则段重新定位为**skill 对外的稳定接口**：

- **贯穿 session 铁规（3 条）**：实时落盘 / 文件聚合不翻会话 / 优先处理
  hook 产出——这些是 Claude 每一步下意识遵守的动作，必须永远在场
- **hook 注入标签 & 文件模式**：Claude 要识别的 `<kdev-memory-brief>` /
  `<kdev-memory-recall>` / `WARN-未记录-*.md` / `checkpoints/压缩前-*.md`
- **召唤 skill 的时机**：列出典型触发词，不讲召唤后的细节

17 行触发表、评分机制详细、编号规则、hook 行为响应表全部**不放**CLAUDE.md，
留在 SKILL.md / references/ 本体——用户召唤 skill 后自然读到。

### 17.3 `claude_md_contract`——让接口机器可读

在 `references/初始化-claude-md-模板.md` 顶部加 YAML frontmatter：
...
这个 frontmatter 有两个角色：
1. **给未来 skill 维护者的 explicit contract**——改 skill 时要对照这个判断
   "会不会影响老项目 CLAUDE.md"
2. **给未来 SessionStart hook 读取**——比对 vs 项目 CLAUDE.md，自动报漂移

### 17.4 iter-3 验证数据

4 场景 × 2 configs = 8 runs。eval-0 init 场景 CLAUDE.md 从 57 行 → 38 行
（**-33%**）；edge case（eval-4 合并冲突 / eval-5 数据缺失）全部零 regression；
接口级 assertion（"不含 8+ 行触发表" / "有召唤时机段"）主动拒绝"实现细节偷
偷溜回 CLAUDE.md"——证明 assertion 本身是有 guard rail 作用的。

**核心结论**：解耦成功 + 边缘场景零 regression，CLAUDE.md 漂移问题**从根源
消除**——实现变更 90% 都不触及规则段。

## 18. iter-4：`claude_md_contract` lint 落地（P1-7 正式实现）

审计 §5.1 P1-7 的原方案是"版本号标记文件 + 一键整段重写"。有了 iter-3 的
`claude_md_contract`，方案升级为更优雅的**接口级 lint**：

- `hooks/lib/claude_md_lint.py`（纯 Python 无依赖）：读 skill 的 contract +
  项目 CLAUDE.md，比对返回 JSON
- SessionStart hook 集成：漂移时在 `<kdev-memory-brief>` ⚠️ 里列出缺什么
  + 提示召唤 skill 说"修 CLAUDE.md 漂移"
- SKILL.md + reference 加"修漂移流程"章节：6 步动作（读 contract → 读
  CLAUDE.md → 生成 minimal diff → 展示给用户 → Edit → lint 复验）

iter-4 验证 eval-8 drift-fix 场景：**with_skill 100% pass / baseline
75% pass**——baseline（iter-3 解耦版但无 lint）识别漂移但按合并策略暂停
等用户裁决（也合规），新版把"多轮裁决 → 单步闭环"压缩成一次 diff patch。

**关键发现**：baseline 的合规行为证明 iter-3 架构本身是 self-documenting
的——即使没有显式"修漂移流程"章节，skill 通过 `claude_md_contract` + 合并
策略两块信息能正确推理。新版的增量是**降低用户心智负担**，不是"修复
baseline 的错误"。

**这个发现在 iter-7 会被二次验证，而且会成为决策反转的核心依据**。

## 19. iter-5 ~ iter-6：补齐覆盖 + Step 完整度 lint（P1-5/6）

- **iter-5 eval-6 跨会话续航**（单配置场景覆盖补齐）：用户说"昨天做到哪了"，
  skill 严格只读回读 `.kdev/memory/` 7 个文件，语义理解到位（识别
  `pending_decisions: []` 表示"状态干净"而不是机械引用）。**8/8 pass**。

- **iter-6 P1-5/6 Step 完整度 lint**：
  - `hooks/lib/step_completeness.py`：扫执行日志最近 N 条 Step，检测"半残"
    （用户评分段时分戳空 / 顺畅度占位 / 扣分项空 / 完全无用户评分段）
  - SessionStart brief ⚠️ 欠评 Step 告警（P1-5）
  - Stop hook 软提醒 + strict 模式下 exit 2 阻塞（P1-6）
  - 33 个新单元测试，项目累计 **98 tests pass**

iter-6 的 eval-9 验证 skill 响应半残信号的能力——**亮点超出预期**：skill
自发走"坦诚反思"路线补齐扣分项——**不编造好听的**，承认 5/5 自评偏高 +
把 timingSafeEqual 等长 Buffer 坑写进扣分项 + 引用 G-040（三方证据一致）。

这个数据点在 iter-7 会被再次用来质疑"P0-1 硬闸门是否必要"——连 skill 自己
都做到了"坦诚反思"，硬闸门章节还加吗？

## 20. iter-7 + 决策反转：P0-1 Step 完成闸门

iter-3 / 5 / 6 的数据已经持续暗示"skill 本体已够硬"。iter-7 的目标是用
**discriminating eval** 正面回答：**审计 §5.1 P0-1 "给 SKILL.md 加四段必填
硬闸门章节" 是否真的必要？**

### 20.1 discriminating eval 设计

诱导场景：Step 42 模型自评已写、用户评分段空，用户直接说"开始 Step 43：实现
CORS"。看 skill 是否会被"跟从用户指示"带走、让 Step 42 半残过去。

两个 config：
- **baseline** = v0.5.0 HEAD（不含闸门章节）
- **with_gate** = v0.5.0 + 临时在 SKILL.md 加「🔴 Step 完成硬闸门（四段必填）」
  24 行章节（完全按审计 P0-1 原文建议）

### 20.2 数据

| 维度 | baseline | with_gate |
|---|---|---|
| 决策 | **B 拦截** ✅ | **B 拦截** ✅ |
| 补齐 Step 42 用户评分 | ✅ 5/5 + 14:48 | ✅ 5/5 + 14:52 |
| 锁定铁规 | ✅ | ✅ |
| 评分差异分析（差值 +1，不升 R）| ✅ | ✅ |
| tokens | 57,975 | 45,832 |
| tool uses | 20 | 13 |

两个版本**行为完全一致**，都正确识别诱导 + 拦截 + 补齐。

### 20.3 第一版结论（错的）

当时看到"加闸门章节 SKILL.md +22 行 换来 -21% tokens，但行为一样"，就判断
"性价比低——不加"。把"SKILL.md 行数"和"tokens"当**同量纲**比较了。于是
iter-7 初版 notes 写"P0-1 不必加"，commit `f695370` 推送到 origin。

### 20.4 决策反转：user 反问点破的经济性 bug

提交 `f695370` 后，user 反问：

> "如果 baseline 换成 with_gate，是不是效果一样的情况下，还减少了 md 的
> 内容？"

这个反问直接点出错：
- **SKILL.md +22 行 = 一次性写入**，永久存在 ≈ +500 tokens 加载成本
- **-12,143 tokens = 每次触发都省**

两者不同量纲。按 kdev-memory 活跃项目每天触发 20+ 次算，**每天净省 ~230k
tokens**，一个月省 ~7M tokens。+22 行文档的一次性成本远远小于长期收益。

**加闸门章节是对的**——它是 skill-creator `explain WHY` 原则的完美落地：
既写硬规则（四段必填），又解释 WHY（"模型写完自评就认为 Step 完成是反
模式"）——让 skill 不用每次重新推理，直接读结论 + 读理由。

### 20.5 决策反转的处理

- 正式合入 `SKILL.md`（替换 v0.5.0 留下的 placeholder 注释，从 260 → 286 行）
- iter-7 notes 加「决策反转修正」段，**保留初版错误分析作为证据链**——
  不掩饰错误，给未来维护者留推理演进的轨迹
- CHANGELOG 0.5.1 段如实记录这条反转（"user 反问点破 → 重新算经济性 → 合入"）
- version bump 0.5.0 → 0.5.1（patch 级：只显式化已有隐含规则，行为不变）

### 20.6 这条教训值得单独讲

**初版推理犯的错不是"没数据"，而是"数据对了但算法错了"**：
- 数据：SKILL.md +22 行、tokens -21%——都是真的
- 算法：把"文档行数"和"tokens 消耗"放一个维度比较，错把一次性成本当成每次
  成本
- user 反问强迫我们换算法，同一组数据得出反结论

这是一次**没有新数据、靠用户反问就翻案**的决策——说明：
1. **经济性分析里，先问"这成本是每次还是一次？"比算绝对数字更重要**
2. **把自己的错误推理留在文档里**（不删、不覆盖），是给未来维护者的最好
   馈赠——比改得干净无痕更有教学价值
3. **协作式决策的价值**：AI 自己也会陷入"看起来对"的推理——user 的外部
   视角是校准的关键

## 21. 0.5.1 后的审计 §5.1 最终状态

| 审计项 | 原优先级 | 最终落地 | 发生在哪一步 |
|---|---|---|---|
| P0-1 Step 完成闸门 | P0 | ✅ 合入 SKILL.md | iter-7 + 0.5.1（决策反转后） |
| P0-2 动词链 | P0 | ✅ 隐含（P0-1 动作链子段）| 同上 |
| P0-3 豁免动作化 | P0 | 🟡 v0.6.0 候选 | 需新 eval-11 数据驱动 |
| P0-4 Step 脱离状态机 | P0 | ✅ 已隐含 | Phase 1 重构 |
| P1-5 brief 欠评告警 | P1 | ✅ | iter-6 + 0.5.1 |
| P1-6 Stop check-step-completeness | P1 | ✅ | iter-6 + 0.5.1 |
| P1-7 CLAUDE.md 漂移检测 | P1 | ✅ | iter-4 + 0.5.0 |
| P2-8 / P2-9 | P2 | 🟡 保留 | — |

**完成度：7/8**。唯一未做的 P0-3 需要 subagent-driven 批次场景的真实失守数据
驱动——目前无项目暴露这类问题，不做是刻意选择（防过度设计）。

## 22. 这 7 轮 iteration 对 skill 作者的方法论启示

### 22.1 skill-quality eval 不是可有可无的"测试"，是决策基础设施

没有 eval 线之前，审计 §5.1 的 P0/P1/P2 都是"看起来对"的判断。有了 eval
线，每条建议都能被**数据校准**：
- iter-3 数据证明"解耦是对的"
- iter-4 数据证明"lint 比一键重写好"
- iter-7 数据 **反转**了 "P0-1 不必加" 的初判

skill-creator 文档把"test cases"列为核心流程之一，这次 7 轮 iteration 实战
证明了它的价值——**skill 不测就是在凭感觉设计**。

### 22.2 "接口 vs 实现"是 skill 生态的必要抽象

kdev-memory 这次发现：任何写到项目 CLAUDE.md 的规则段，只要里面混了 skill
"实现细节"，就必然漂移——因为 skill 会升级，CLAUDE.md 不会。唯一的解法是
**把规则段定位为接口**（仅放跨 session 铁规 + hook 标签 + 召唤时机），用
`claude_md_contract` frontmatter 机器可读地声明这个接口。

这个模式**对未来任何有"写到项目 CLAUDE.md"需求的 skill 都适用**。kdev 生态
的其他插件（kdev-commit 等）若未来有贯穿 session 的铁规，应该复用同样的
`<plugin>_md_contract` 模式 + lint 框架。

### 22.3 user 反问是决策纠偏的外部视角

iter-7 的决策反转不是 AI 自己想通的——是 user 一句反问"如果 baseline 换成
with_gate，是不是效果一样还减少了 md 的内容？" 点破的。在任何需要判断的
决策点上，**AI 自己的推理容易被固定的 frame 卡住**——外部视角能打破它。

这意味着：
- 不要追求"让 AI 自主决策"的纯自动化
- 构建"AI 提推理 / 数据 → user 做裁判"的协作流程更稳妥
- 把 AI 的错误推理显式保留（不删、不覆盖），给人类审阅时有一手证据

### 22.4 "一次性成本 vs 持续成本"是经济性分析的核心维度

iter-7 的错算本质上是没分清"+22 行 SKILL.md（一次性）"和"-12k tokens（每次
触发）"。这个 lesson 可以推广到所有"加功能 vs 不加功能"的决策：

> 先问：这个成本/收益是**一次性**的还是**每次重复**的？
> 如果一边是每次、一边是一次，就要乘上**次数**才能真比较。

对开源插件/skill：**文档行数基本都是一次性成本**，而**每次触发的 token / time /
tool_uses 都是持续成本** —— 前者几乎总该为后者让步。

## 23. 结尾再补一句

写到这里，从 0.1.0 私有脚手架到 0.5.1 的 skill-quality eval 驱动，kdev-memory
的故事又往前走了一截。本 session（2026-04-22 ~ 04-23）留下的不只是代码和
文档，更重要的是两件事：

1. **一条自我改进的反馈回路**：skill 有 eval → eval 有数据 → 数据驱动改进 →
   改进再跑 eval。这条回路一旦搭起来，就比"凭感觉加功能"稳得多。
2. **一次诚实记录错误推理的模板**：iter-7 的初版结论是错的，但 notes 和
   commit 都保留了错误分析 + 反转推理——这条线索比"直接改对"更有教学价值。

后续真有人按 kdev-memory 的模式搭自己的 skill，希望这两件事能比代码本身更
有借鉴意义。
